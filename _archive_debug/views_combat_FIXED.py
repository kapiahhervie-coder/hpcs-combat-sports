...
def get_atlet_boxing(user):
    ...
    semua view/audit di bawah app 'combat' HANYA melihat atlet cabang
    Boxing, bukan seluruh cabang (Muay Thai, Karate, dst).
    """
...
class DashboardBoxingView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        daftar_atlet = get_atlet_boxing(request.user).order_by('nama_atlet')  # FIX: filter cabang='boxing'
...
class L1CorrectionView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        atlet_qs   = get_atlet_boxing(request.user)  # FIX: filter cabang='boxing'
    ...
    def post(self, request):
        try:
            ...
            atlet    = get_object_or_404(Atlet, pk=atlet_id, cabang='boxing') if atlet_id else None  # FIX
...
def hapus_l1_audit(request, pk):
    audit = get_object_or_404(CorrectionAuditL1, pk=pk, atlet__cabang='boxing')  # FIX
...
class L2StrengthView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        atlet_qs   = get_atlet_boxing(request.user)  # FIX
...
def hapus_l2_audit(request, pk):
    audit = get_object_or_404(StrengthAuditL2, pk=pk, atlet__cabang='boxing')  # FIX
    nama  = audit.atlet_name
    audit.delete()
...
def hapus_l3_audit(request, pk):
    audit = get_object_or_404(PowerAuditL3, pk=pk, atlet__cabang='boxing')  # FIX
    nama  = audit.atlet_name
    audit.delete()
...
def hapus_l4_audit(request, pk):
    audit = get_object_or_404(SpeedAgilityAuditL4, pk=pk, atlet__cabang='boxing')  # FIX
    nama  = audit.atlet_name
    audit.delete()
