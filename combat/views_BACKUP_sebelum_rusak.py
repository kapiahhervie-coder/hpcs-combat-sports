...
class DashboardView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        distribusi_cabor = [
            {
                'cabor': {'nama': 'Boxing'},
                'total': Atlet.objects.filter(cabang__iexact='boxing').count(),
                'rata_level': 'L2',
                'status': 'Baik',
                'pct_audit': 75,
            },
            {
                ...
                'total': Atlet.objects.filter(cabang__iexact='Muay Thai').count(),
                'rata_level': '-',
                'status': 'Cukup',
                'pct_audit': 0,
            },
            {
                'cabor': {'nama': 'Karate'},
                'total': Atlet.objects.filter(cabang__iexact='karate').count(),
                'rata_level': '-',
                'status': 'Cukup',
                'pct_audit': 0,
            },
            {
                ...
                'total': Atlet.objects.filter(cabang__iexact='taekwondo').count(),
                'rata_level': '-',
                'status': 'Cukup',
                'pct_audit': 0,
            },
...
class L1CorrectionView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        history    = CorrectionAuditL1.objects.order_by('-timestamp')[:50]
        atlet_list = Atlet.objects.all().order_by('nama_atlet')
...
class L2StrengthView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        history    = StrengthAuditL2.objects.order_by('-timestamp')[:50]
        atlet_list = Atlet.objects.all().order_by('nama_atlet')
        return render(request, self.template_name, {
            'history':    history,
            'atlet_list': atlet_list,
...
class L3PowerView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        history    = PowerAuditL3.objects.order_by('-timestamp')[:50]
        atlet_list = Atlet.objects.all().order_by('nama_atlet')
        return render(request, self.template_name, {
            'history':    history,
            'atlet_list': atlet_list,
...
class L4SpeedAgilityView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        history    = SpeedAgilityAuditL4.objects.order_by('-timestamp')[:50]
        atlet_list = Atlet.objects.all().order_by('nama_atlet')
        return render(request, self.template_name, {
            'history':    history,
            'atlet_list': atlet_list,
...
class ReportCardView(LoginRequiredMixin, View):
    ...
    def get(self, request, atlet_id):
        ...
        radar = {
            "l1": round(l1.total_skor, 1) if l1 else 0,
            "l2": round(l2.total_skor, 1) if l2 else 0,
            "l3": round(l3.total_skor, 1) if l3 else 0,
            "l4": round(l4.total_skor, 1) if l4 else 0,
        }

        scores_ada = [v for v in radar.values() if v > 0]
        ...
        level_summary = [
            ("l1", l1, "#00d2ff", "L1 Correction"),
            ("l2", l2, "#00ff88", "L2 Strength"),
            ("l3", l3, "#ffb300", "L3 Power"),
            ("l4", l4, "#ff6b9d", "L4 Speed Agility"),
        ]
...
class ReportCenterView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        atlet_qs = Atlet.objects.filter(aktif=True) if hasattr(Atlet, 'aktif') else Atlet.objects.all()
